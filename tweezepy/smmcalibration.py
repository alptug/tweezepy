...
class calibration(MLEfit):
    ...
    def plot(self, 
        ...
        eb = ax[0].errorbar(data['x'], data['y'], yerr=data['yerr'],
                            ...
                            ms=8, capsize = 5, label = data_label)
        ax[0].set_xscale('log')
        ax[0].set_yscale('log')
    ...
    def _make_noise_guess(self,noise,fsample):
        ...
        A = np.vstack([np.ones_like(psd.data['x']),1/psd.data['x'],1/psd.data['x']**2]).T
    ...
    def mlefit(self,
        ...
        if isinstance(self,HV):
            ...
            if fitfunc == 'SMMHV':
                ...
                minimizer_kwargs.update({'bounds':[(0,np.inf),(0,np.inf),(0,np.inf)]})
                if not guess:
        ...
        if not guess:
            ...
            if tracking_error and epsilon is None:
                guess.append(0.)
        self.guess = guess
        MLEfit.__init__(self, pedantic = pedantic, scale_covar = scale_covar, minimizer_kwargs=minimizer_kwargs)
    @property
    def data(self):
        """
...
class HV(calibration):
    ...
    def __init__(self, trace, fsample,taus = 'octave',mode = 'ohvar',edf='real'):
        calibration.__init__(self,trace,fsample)
        assert taus in ['all','octave','decade'], "taus must be either all, octave, or decade."
        assert mode in ['hvar','ohvar','totvar'], "mode must be either hvar, ohvar, or totvar."
        assert edf in ['real','approx'], "edf must be either real or approx."
        if mode == 'hvar':
            taus, edfs, ohvs = hvar(trace, rate = fsample, taus = taus,edf=edf, overlapping = False)        
        elif mode == 'ohvar':
            taus, edfs, ohvs = hvar(trace, rate = fsample, taus = taus,edf=edf, overlapping = True)     
        elif mode == 'totvar':
            #taus, edfs, oavs = totvar(trace, rate = fsample, taus = taus,edf=edf)
